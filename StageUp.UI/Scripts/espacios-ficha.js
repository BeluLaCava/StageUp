(function () {
    ...
    upload.addEventListener("change", function () {
        ...
        if (!files.length) { preview.src = originalSource; return; }
        if (files.length > 8 || files.some(function (file) { return file.size > 3 * 1024 * 1024 || !/\.(jpe?g|png)$/i.test(file.name); })) {
            upload.value = "";
            preview.src = originalSource;
            document.getElementById("photo-error").textContent = "Elegí hasta ocho imágenes JPG o PNG de hasta 3 MB cada una.";
            return;
        }
        files.forEach(function (file, index) {
            ...
            if (index === 0) preview.src = objectUrl;
        });
    });
    render();
    showPrice();
