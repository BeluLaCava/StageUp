...
<asp:Content ID="SpaceDetailContent" ContentPlaceHolderID="MainContent" runat="server">
        ...
        <div class="space-detail-container">
            ...
            <asp:Panel ID="pnlDetalleEspacio" runat="server" CssClass="space-detail" Visible="false">
                <asp:Panel ID="pnlGaleria" runat="server" CssClass="space-gallery" data-space-gallery="true">
                    ...
                    <div class="space-gallery-thumbnails" aria-label="Fotografías del espacio">
                        <asp:Repeater ID="rptGaleriaFotos" runat="server">
                            <ItemTemplate>
                                <button type="button" class="space-gallery-thumbnail" data-gallery-thumbnail data-src='<%# ResolveUrl(Container.DataItem.ToString()) %>' aria-label='<%# "Ver fotografía " + (Container.ItemIndex + 1) %>'>
                ...
                <asp:Panel ID="pnlGaleriaVacia" runat="server" CssClass="space-gallery-empty">
                    <span aria-hidden="true">◇</span><strong>Este espacio todavía no tiene fotografías</strong><small>La información disponible se muestra a continuación.</small>
                </asp:Panel>
                ...
                <div class="space-detail-layout">
                    <div class="space-detail-main">
                        ...
                        <section class="space-host-card" aria-labelledby="space-host-title">
                            ...
                            <div><span class="eyebrow">Gestionado por</span><h2 id="space-host-title"><asp:Literal ID="litNombreGestor" runat="server" /></h2><div class="space-host-rating" aria-label="Este gestor todavía no tiene calificaciones"><span aria-hidden="true">☆☆☆☆☆</span> Sin calificaciones todavía</div></div>
                        ...
                        <section class="space-detail-section" aria-labelledby="space-description-title">
                            <span class="eyebrow">Conocé el lugar</span><h2 id="space-description-title">Sobre el espacio</h2>
                        ...
                        <asp:Panel ID="pnlFicha" runat="server">
                            <section class="space-detail-section" aria-labelledby="space-info-title">
                                <span class="eyebrow">Información esencial</span><h2 id="space-info-title">Todo lo que necesitás saber</h2>
                                <div class="space-information-grid">
                                    ...
                                    <article><span class="space-information-icon" aria-hidden="true">$</span><small>Valor de referencia</small><strong><asp:Literal ID="litPrecioHora" runat="server" /></strong></article>
                                </div>
                            </section>

                            <section class="space-detail-section" aria-labelledby="space-features-title">
                                <span class="eyebrow">Incluido en el espacio</span><h2 id="space-features-title">Características y equipamiento</h2>
                                <asp:Panel ID="pnlCaracteristicas" runat="server" CssClass="space-feature-grid"><asp:Repeater ID="rptEquipamiento" runat="server"><ItemTemplate><span><span aria-hidden="true">✓</span> <%#: Container.DataItem %></span></ItemTemplate></asp:Repeater></asp:Panel>
                                ...
                                <asp:Panel ID="pnlDetalleEquipamiento" runat="server" CssClass="space-equipment-detail"><strong>Detalle del equipamiento</strong><p><asp:Literal ID="litDetalleEquipamiento" runat="server" /></p></asp:Panel>
                            </section>

                            <section class="space-detail-section" aria-labelledby="space-availability-title">
                                <span class="eyebrow">Horarios habituales</span><h2 id="space-availability-title">Disponibilidad informada</h2>
                                <p class="space-detail-intro">Estos horarios son orientativos. Antes de enviar una solicitud se vuelve a comprobar que la franja continúe libre.</p>
                                <asp:Panel ID="pnlDisponibilidad" runat="server" CssClass="space-availability-list"><asp:Repeater ID="rptDisponibilidad" runat="server"><ItemTemplate><div><span aria-hidden="true">◷</span><%#: Container.DataItem %></div></ItemTemplate></asp:Repeater></asp:Panel>
                        ...
                        <asp:Panel ID="pnlSinFicha" runat="server" CssClass="space-detail-section space-profile-pending">
                            <span class="eyebrow">Ficha en preparación</span><h2>Próximamente vas a ver más información</h2><p>El gestor todavía no cargó ubicación, capacidad, precio, disponibilidad y equipamiento para este espacio.</p>
                        </asp:Panel>

                        <section class="space-detail-section" aria-labelledby="space-reviews-title">
                            <span class="eyebrow">Experiencias verificadas</span><h2 id="space-reviews-title">Reseñas del espacio</h2>
                            <div class="space-reviews-empty"><span class="space-reviews-stars" aria-hidden="true">☆☆☆☆☆</span><strong>Todavía no hay reseñas</strong><p>Las calificaciones aparecerán después de reservas finalizadas, cuando esta función esté habilitada.</p></div>
                        </section>
                    </div>

                    <aside class="reservation-card" aria-label="Planificar solicitud de reserva">
                        <span class="reservation-card-label">Planificá tu uso</span><div class="reservation-card-price"><asp:Literal ID="litPrecioReserva" runat="server" /></div>
                        <p>Elegí una fecha y una franja para conocer el importe estimado.</p>
                        ...
                        <asp:Panel ID="pnlPlanificadorDisponible" runat="server" CssClass="reservation-planner" data-reservation-planner="true">
                            <div class="form-field"><label for="reservation-date">Fecha</label><input id="reservation-date" type="date" /></div>
                            ...
                            <div class="reservation-estimate" aria-live="polite"><span>Importe estimado</span><strong id="reservation-total">—</strong><small id="reservation-summary">Se calcula según la duración elegida.</small></div>
                        </asp:Panel>
                        <asp:Panel ID="pnlPlanificadorNoDisponible" runat="server" CssClass="reservation-unavailable">El gestor todavía no configuró horarios y precio. Podrás solicitar cuando esa información esté disponible.</asp:Panel>
                        <asp:Panel ID="pnlReservaInvitado" runat="server"><a class="button button-primary button-full" href="../IniciarSesion.aspx">Iniciar sesión para solicitar</a><small>Necesitás una cuenta activa para enviar la solicitud.</small></asp:Panel>
                        <asp:Panel ID="pnlReservaAutenticado" runat="server"><button class="button button-primary button-full" type="button" disabled>Enviar solicitud próximamente</button><small>El envío real se habilitará al conectar el circuito de reservas.</small></asp:Panel>
    ...
    <dialog id="space-gallery-lightbox" class="space-gallery-lightbox" aria-label="Fotografía ampliada">
        <button type="button" class="space-lightbox-close" aria-label="Cerrar fotografía" data-gallery-close>×</button><img data-gallery-lightbox-image alt="Fotografía ampliada del espacio" />
    </dialog>
</asp:Content>

<asp:Content ID="SpaceDetailScripts" ContentPlaceHolderID="PageScripts" runat="server">
    <script src="<%= ResolveUrl("~/Scripts/detalle-espacio.js") %>?v=1"></script>
</asp:Content>
