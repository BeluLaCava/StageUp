...
=======
        ...
        public ResultadoOperacion ValidarFicha(EspacioArtistico espacio)
        {
            ...
            if (ficha.FotosRutas == null || ficha.FotosRutas.Count > 8)
                return ResultadoOperacion.Error("Podés cargar hasta ocho fotografías del espacio.");
            var fotosSeleccionadas = new HashSet<string>();
            foreach (string fotoRuta in ficha.FotosRutas)
                if (string.IsNullOrEmpty(fotoRuta) ||
                    !Regex.IsMatch(fotoRuta, @"^~/Content/Uploads/Espacios/[a-f0-9]{32}\.jpg$") ||
                    !fotosSeleccionadas.Add(fotoRuta))
                    return ResultadoOperacion.Error("Hay fotografías no válidas o repetidas.");
            if (!string.IsNullOrEmpty(ficha.FotoRuta) && fotosSeleccionadas.Count > 0 && !fotosSeleccionadas.Contains(ficha.FotoRuta))
                return ResultadoOperacion.Error("La foto de portada debe formar parte de la galería.");
